...
class MessagesRepository:
    ...
    def insert_a_document(self, collection_name: str, document_json: str):
        collection = self.mongo_db[collection_name]
        ...
        if len(str(object_id)) == 24:
            ...
            message = f"Delete a document failed. document content: {document_json}"
    ...
    def delete_one(self, collection_name: str, query_document: dict) -> dict:
        ...
        if result["n"] == 1 and result["ok"] == 1:
            ...
            message = f"Delete a document failed, total found items: {result['n']}, delete succeed: {result['ok']}."
            raise Exception(message)
    ...
    def update_one(self, collection_name: str, query_document: dict, update_content: dict) -> dict:
        ...
        try:
            ...
            if result["n"] == 1 and result["ok"] == 1:
                ...
                message = f"Update a document failed, total found items: {result['n']}, delete succeed: {result['ok']}."
                raise Exception(message)
